...
export const getTranslatedVideos = video => async dispatch => {
  console.log("-------", JSON.stringify(video));
  if (video.parentid || video.id) {
    const parentId = video.parentid.length > 0 ? video.parentid : video.id;
    console.log("PARENT ID:", parentId);
    const res = await api.get(`videos/${parentId}/translatedvideos`);
    const videos = res.data.data;
    console.log("GET TRANSLATED VIDEOS:", res);
    dispatch({
      type: GET_TRANSLATED_VIDEOS,
      payload: { videos },
    });
  }
};
