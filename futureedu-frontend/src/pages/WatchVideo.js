...
const WatchVideo = ({
  ...
  const [cVideoId, setcVideoId] = useState(videoId);
    ...
    >
      <div className="video-container">
        ...
        <div className="video-info">
          ...
          <div>
            <h4>Select Video Language</h4>
            <select
              value={selectedLanguage}
              onChange={event => {
                console.log(event.target.value);
                const selectedVideo = tvideos.filter(video => {
                  return video.language == event.target.value;
                });
                if (selectedVideo.length > 0) {
                  console.log(selectedVideo[0]);
                  setSelectedLanguage(event.target.value);
                  setcVideoId(selectedVideo[0].id);
                } else {
                  setSelectedLanguage(event.target.value);
                  setcVideoId(videoId);
                }
            ...
            >
              ...
              {tvideos &&
                tvideos.map(video => {
                  return (
                    <option value={video.language}>{video.language}</option>
                  );
                })}
            </select>
          </div>
...
const mapStateToProps = ({
  ...
  notfound,
  tvideos: translatedVideos.videos,
  tfetching: translatedVideos.isFetchingV,
});
